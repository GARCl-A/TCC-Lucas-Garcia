...
def load_and_validate_data():
    ...
    required_cols = ['Lambda', 'CVaR_Mi', 'Retorno_Mi', 'Vol_Hedge_MW']
    
    if not all(col in df.columns for col in required_cols):
        raise ValueError(f"Colunas obrigatórias ausentes: {required_cols}")
...
def create_efficient_frontier_plot(df):
    ...
    for i, (bar, vol) in enumerate(zip(bars, df['Vol_Hedge_MW'])):
        if vol > 0:
            ax2.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(df['Vol_Hedge_MW'])*0.01,
...
def add_statistical_annotations(df, ax):
    risk_range = df['CVaR_Mi'].max() - df['CVaR_Mi'].min()
    return_range = df['Retorno_Mi'].max() - df['Retorno_Mi'].min()
