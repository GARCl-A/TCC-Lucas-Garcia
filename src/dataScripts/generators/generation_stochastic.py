...
RAW_DIR = os.path.normpath(
    os.path.join(BASE_DIR, "..", "..", "..", "data", "ccee", "geracao_usina")
)
...
MAPA_SUBMERCADOS = {"SUDESTE": "SE", "SUL": "S", "NORDESTE": "NE", "NORTE": "N"}

# Lista de usinas para filtrar (None = todas as usinas)
USINAS_FILTRO = [8578, 110222, 81285]
...
def carregar_dados_historicos():
    ...
    df_completo["SUBMERCADO"] = (
        df_completo["SUBMERCADO"]
        .map(MAPA_SUBMERCADOS)
        .fillna(df_completo["SUBMERCADO"])
    )

    # Extrair mês (1-12)
    df_completo["mes"] = df_completo["MES_REFERENCIA"].astype(str).str[-2:].astype(int)

    # Filtrar usinas se lista fornecida
    if USINAS_FILTRO is not None:
        df_completo = df_completo[df_completo["COD_ATIVO"].isin(USINAS_FILTRO)]
        print(f"🔍 Filtrado para {len(USINAS_FILTRO)} usinas específicas")
...
def calcular_estatisticas_mensais(df_historico):
    ...
    stats = (
        df_historico.groupby(
            ["COD_ATIVO", "SUBMERCADO", "FONTE_ENERGIA_PRIMARIA", "mes"]
        )["GERACAO_CENTRO_GRAVIDADE"]
...
def gerar_cenarios_estocasticos(stats, df_pld):
    ...
    for i, ((data, submercado), grupo_pld) in enumerate(grupos, 1):
        ...
        stats_filtrado = stats[
            (stats["SUBMERCADO"] == submercado) & (stats["mes"] == mes)
        ].copy()
        ...
        # Combinar
        df_chunk = pd.concat(
            [pld_rep.reset_index(drop=True), stats_rep.reset_index(drop=True)], axis=1
        )

        # Ajustar média por fonte primária (vetorizado)
        mask_hidro = df_chunk["FONTE_ENERGIA_PRIMARIA"].str.contains(
            "Hidráulica|Hidraulica", case=False, na=False
        )
        mask_eolica = df_chunk["FONTE_ENERGIA_PRIMARIA"].str.contains(
            "Eólica|Eolica", case=False, na=False
        )

        df_chunk["Media_Ajustada"] = df_chunk["Media_Historica"]

        df_chunk.loc[mask_hidro, "Media_Ajustada"] = df_chunk.loc[
            mask_hidro, "Media_Historica"
        ] * (1.0 / df_chunk.loc[mask_hidro, "Fator_PLD"])

        df_chunk.loc[mask_eolica, "Media_Ajustada"] = df_chunk.loc[
            mask_eolica, "Media_Historica"
        ] * (df_chunk.loc[mask_eolica, "Fator_PLD"] ** 0.5)

        # Sorteio de Monte Carlo
        df_chunk["geracao_mwm"] = np.maximum(
            0.0,
            np.random.normal(
                loc=df_chunk["Media_Ajustada"], scale=df_chunk["Desvio_Padrao"]
            ),
        ).round(2)

        # Selecionar colunas
        resultado_chunk = df_chunk[
            ["data", "submercado", "COD_ATIVO", "cenario", "geracao_mwm"]
        ].copy()
