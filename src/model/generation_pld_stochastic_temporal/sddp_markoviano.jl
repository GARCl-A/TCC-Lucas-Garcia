...
function preprocess_data(config::SDDPConfig, data::MarketData)
    ...
    # Agrega geração por (data, cenario, submercado) diretamente no DataFrame
    ...
    # Filtra para o horizonte e cenários selecionados de uma vez
    set_meses    = Set(meses)
    ...
    # Índices O(1) sobre os DataFrames já filtrados
...
function build_sddp_model(meses, cenarios_selecionados, dados_por_mes, trajetorias)
    ...
    model = SDDP.PolicyGraph(
        ...
        # Print throttling: atualiza a cada 500 nós para não travar o terminal
        ...
        # Objetivo: lucro total - multa por falência
        @stageobjective(sp,
            fluxo_contratos +
