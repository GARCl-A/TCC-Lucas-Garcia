...
\begin{itemize}
    ...
    \item $\mathbf{P}^{(t)} \in \mathbb{R}^{3 \times 3}$: Matriz de transição dependente do
    tempo, onde $P^{(t)}_{ij} = \Pr(m_{t+1} = j \mid m_t = i)$. Para transições em direção
    a estágios com variabilidade climática ($t+1$ não degenerado), utiliza-se a matriz
    empírica $\mathbf{P}^{emp}$ estimada das trajetórias históricas. Para transições
    direcionadas a estágios degenerados ($t+1$ degenerado), utiliza-se a matriz absorvente
    $\mathbf{P}^{deg}$ que colapsa todas as probabilidades para o estado base
    ($P^{deg}_{i1} = 1,\; P^{deg}_{ij} = 0\; \forall j \neq 1$).
    % build_transition_matrices → Pt[t] = degenerado[t+1] ? P_deg : P_empirica
...
($m = 1$), e os estados $m \in \{2, 3\}$ ficam vazios ($\Omega_{t,m} = \emptyset$). A
adoção de matrizes de transição não-estacionárias $\mathbf{P}^{(t)}$ garante que a
probabilidade de transição para estados inexistentes seja estritamente nula, preservando
formalmente a independência condicional do processo de decisão de Markov (MDP). Os estados
vazios recebem um ruído \emph{dummy} de zeros exclusivamente para satisfazer a tipagem do
\texttt{SDDP.parameterize}; como a probabilidade de acesso a esses estados é zero pela
construção de $\mathbf{P}^{(t)}$, eles nunca são avaliados na função objetivo e não
introduzem qualquer distorção estatística no modelo.
...
\subsection{Estimação da Matriz Empírica}

A matriz empírica $\mathbf{P}^{emp}$ é estimada por contagem de frequências nas trajetórias
históricas, considerando apenas transições entre estágios não degenerados:

\begin{align}
    N_{ij} = \sum_{c \in \mathcal{C}} \sum_{t=1}^{T-1}
    \mathbf{1}\bigl[m_t(c) = i\bigr] \cdot \mathbf{1}\bigl[m_{t+1}(c) = j\bigr]
    \cdot \mathbf{1}\bigl[\neg\,\text{deg}(t)\bigr] \cdot \mathbf{1}\bigl[\neg\,\text{deg}(t+1)\bigr]
\end{align}
...
%   if !degenerado[t] && !degenerado[t+1]: contagens[seq[t], seq[t+1]] += 1
%   P_empirica[i,:] = contagens[i,:] / total   (ou 1/N_ESTADOS se total == 0)

O vetor de matrizes $\mathbf{P}^{(t)}$ é então montado como:

\begin{align}
    \mathbf{P}^{(t)} =
    \begin{cases}
        \mathbf{P}^{deg} & \text{se } \text{deg}(t+1) \\
        \mathbf{P}^{emp} & \text{caso contrário}
    \end{cases}
    \quad t = 1, \ldots, T-1
\end{align}
...
A probabilidade de transição do nó $(t, m)$ para o nó $(t+1, m')$ é $P^{(t)}_{mm'}$.
% SDDP.MarkovianGraph(stages=T, transition_matrix=Pt, root_node_transition=root_probs)
...
\begin{enumerate}
    \item \textbf{Nível Markoviano}: qual estado $m_{t+1}$ será atingido, governado por $\mathbf{P}^{(t)}$.
...
\begin{itemize}
    ...
    \item Cenários históricos usados para estimar $\mathbf{P}^{(t)}$ e $\Omega_{t,m}$: 2000.
    % num_cenarios = 2000
